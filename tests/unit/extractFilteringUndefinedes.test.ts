...
describe('nested objects are filtered for empties', () => {
  test('simple nested object should be empty', () => {
    ...
    expect(util.inspect(extractFilteringEmpties<{foo: {bar?: number, avail: string}}>({foo: {bar: jpv('foo'), avail: jpv('avail')}}, [null, undefined], {avail: 'v'})))
      .toEqual('{ foo: { avail: \'v\' } }');
  });
});
...
describe('fullExtractorMap', () => {
  ...
  it('should return { foo: 1 } for { body: "1" }, { foo: jpv("body", x => parseInt(x, 10) }', () => {
    expect(extractFilteringEmpties<{foo: number}>({foo: jpv('body', (x: string) => parseInt(x, 10))}, [undefined], {body: '1'})).toEqual({foo: 1});
  });
  it('should return { foo: 1 } for { body: 1 }, {foo: jpv("body""foo")}', () => {
    expect(extractFilteringEmpties<{foo: number}>({foo: jpv('body.foo')}, [undefined], {body: {foo: 1}})).toEqual({foo: 1});
  });
  it('should return { foo: 1 } for { body: 1 }, {foo: 1 }', () => {
    expect(extractFilteringEmpties<{foo: number}>({foo: constant(1)}, [undefined], {body: {foo: 5}})).toEqual({foo: 1});
  });
  it('should return { foo: [1, 2, 3] } for { foo: jpv("body") }, { body: [1, 2, 3] }', () => {
    expect(extractFilteringEmpties<{foo: number}>({foo: jpv('body')}, [undefined], {body: [1, 2, 3]})).toEqual({foo: [1, 2, 3]});
  });
  it('should return { foo: [1, 2, 3] } for { foo: jpq("$..blub") }, { body: { blub: 1, key1: {blub: 2}, key2: {blub: 3}}}}', () => {
    expect(extractFilteringEmpties<{foo: number[] }>({foo: jpq('$..blub')}, [undefined], {
      body: {
        blub: 1,
        key1: {blub: 2},
        key2: {blub: 3}
  ...
  it('should return { foo: [1, 2, 3] } for { foo: jpv("bodyblub") }, { body: { blub: 1, key1: {blub: 2}, key2: {blub: 3}}}}', () => {
    expect(extractFilteringEmpties<{foo: number}>({foo: jpv('body.blub')}, [undefined], {
      body: {
        blub: 1,
        key1: {blub: 2},
        key2: {blub: 3}
  ...
  it('should return { foo: 2 } for { foo: jpv("body"".[?(@.blub > 1)].blub") }, { body: { blub: 1, key1: {blub: 2}, key2: {blub: 3}}}}', () => {
    expect(extractFilteringEmpties<{foo: number}>({foo: jpv('body.[?(@.blub > 1)].blub')}, [undefined], {
      body: {
        blub: 1,
        key1: {blub: 2},
        key2: {blub: 3}
  ...
  it('should return { foo: 6 } for { foo: jpc("$..[?(@.blub > 1)].blub") }, { body: { blub: 1, key1: {blub: 2}, key2: {blub: 3}}}}', () => {
    expect(extractFilteringEmpties<{foo: number[]}>({foo: jpq('$..[?(@.blub > 1)].blub')}, [undefined], {
      body: {
        blub: 1,
        key1: {blub: 2},
        key2: {blub: 3}
  ...
  it('should return { foo: 6 } for { foo: jpq("$..blub", x => parseInt(x, 10)) }, { body: { blub: "1", key1: {blub: "2"}, key2: {blub: "3"}}}}', () => {
    expect(extractFilteringEmpties<{foo: number[]}>({foo: jpq('$..blub', (x) => parseInt(x, 10))}, [undefined], {
      body: {
        blub: '1',
        key1: {blub: '2'},
        key2: {blub: '3'}
  ...
  it('should return { foo: 6 } for { foo: jpa("$..blub", x => x.reduce((a, b) => a + b, 0)) }, { body: { blub: "1", key1: {blub: "2"}, key2: {blub: "3"}}}}', () => {
    expect(extractFilteringEmpties<{foo: number}>({foo: jpa('$..blub', (x) => x.reduce((a, b) => a + b, 0))}, [undefined], {
      body: {
        blub: 1,
        key1: {blub: 2},
        key2: {blub: 3}
  ...
  it('should return { foo: 6 } for { foo: jpa("$..blub", x => x.reduce((a, b) => a * b, 1)) }, { body: { blub: "1", key1: {blub: "2"}, key2: {blub: "3"}}}}', () => {
    expect(extractFilteringEmpties<{foo: number}>({foo: jpa('$..blub', (x) => x.reduce((a, b) => a * b, 1))}, [undefined], {
      body: {
        blub: 1,
        key1: {blub: 2},
        key2: {blub: 3}
  ...
  it('should return { foo: 6 } for { foo: jpa("$..blub", x => x.reduce((a, b) => a * b, 1)) }, { body: { blub: "1", key1: {blub: "2"}, key2: {blub: "3"}}}} in curried form', () => {
    expect(extractFilteringEmpties<{foo: {bar: number } }>({foo: jpa('$..blub', (x) => x.reduce((a, b) => a * b, 1))}, [undefined])({
      body: {
        blub: 1,
        key1: {blub: 2},
        key2: {blub: 3}
  ...
  it('should return { } for { foo: jpv("bar"), bar: jpv("foo") }, { foo: 1, bar: undefined } relying on defaults', () => {
    expect(extractFilteringEmpties<{foo: number, bar?: any }>({foo: jpv('bar'), bar: jpv('foo')}, {foo: 1, bar: undefined})).toEqual({bar: 1});
  });
  ...
  it('should return { } for { foo: "Stryker was here" }, { foo: jpv("foo") } relying on defaults', () => {
    expect(extractFilteringEmpties<{foo: number }>({foo: jpv('foo')}, {foo: 'Stryker was here'})).toEqual({foo: 'Stryker was here'});
  });
  ...
  it('should return { foo: 1 } for { body: {foo: null} }, {foo: jpv("body.foo")} relying on defaults', () => {
    expect(extractFilteringEmpties<{foo: number }>({foo: jpv('body.foo')}, {body: {foo: null}})).toEqual({foo: null});
  });
  ...
  it('should return { foo: 1 } for { body: {foo: empty} }, {foo: jpv("body.foo")} with strange empties', () => {
    expect(extractFilteringEmpties<{foo: number }>({foo: jpv('body.foo')}, ['empty'], {body: {foo: 'empty'}})).toEqual({});
  });
  ...
  it('should return { foo: 1 } for { body: {foo: null} }, {foo: jpv("body.foo")}', () => {
    expect(extractFilteringEmpties<{foo: number }>({foo: jpv('body.foo')}, [undefined, null], {body: {foo: null}})).toEqual({});
  });
  ...
  it('should return { foo: 1 } for { body: {foo: null} }, {foo: jpv("body.foo")}', () => {
    expect(extractFilteringEmpties<{foo: number }>({foo: jpv('body.foo')}, [null], {body: {foo: null}})).toEqual({});
  });

  it('should return { } for { foo: null }, { foo: jpv("foo") }', () => {
    expect(extractFilteringEmpties<{foo: number }>({foo: jpv('foo')}, [null], {foo: undefined})).toEqual({foo: undefined});
  });
  ...
  it('should return { } for { foo: jpv("bar") }, { bar: undefined }', () => {
    expect(extractFilteringEmpties<{foo: number }>({foo: jpv('bar')}, [null], {bar: undefined})).toEqual({foo: undefined});
  });
  it('should return { foo: 1 } for { body: {foo: null} }, {foo: jpv("body.foo")}', () => {
    expect(extractFilteringEmpties<{foo: number }>({foo: jpv('body.foo')}, [null], {body: {foo: undefined}})).toEqual({foo: undefined});
  });

  it('should return { } for { foo: undefined }, { foo: jpv("foo") }', () => {
    expect(extractFilteringEmpties<{foo: number }>({foo: jpv('foo')}, [undefined, null], {foo: undefined})).toEqual({});
  });
  it('should return { } for { foo: jpv("bar") }, { bar: undefined }', () => {
    expect(extractFilteringEmpties<{foo: number }>({foo: jpv('bar')}, [undefined, null], {bar: undefined})).toEqual({});
  });
  it('should return { foo: 1 } for { body: {foo: undefined} }, {foo: jpv("body.foo")}', () => {
    expect(extractFilteringEmpties<{foo: number }>({foo: jpv('body.foo')}, [undefined, null], {body: {foo: undefined}})).toEqual({});
  });

  it('should return { foo: { bar: 5 } for { foo: { bar: jpv("body") }}, { body: 5 } in curried form relying on defaults', () => {
    expect(extractFilteringEmpties<{foo: {bar: number} }>({foo: {bar: jpv('body')}})({body: 5})).toEqual({foo: {bar: 5}});
  });
  ...
  it('should return { foo: { bar: 5 } for { foo: { bar: jpv("body") }}, { body: 5 } in curried form', () => {
    expect(extractFilteringEmpties<{foo: { bar: number } }>({foo: {bar: jpv('body')}}, [undefined, null])({body: 5})).toEqual({foo: {bar: 5}});
  });
