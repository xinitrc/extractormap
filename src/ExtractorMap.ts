...
function _extractForKeyPartial<T, S extends Record<string, any>>(valuesInterpretedAsEmpty: unknown[], extractionMap: ExtractorMap<T, S>, extractionKey: keyof T, inputObject: S): DeepPartial<T[keyof T]> {
    ...
    const generateExtractor: (input: TypeExtractor<T[typeof extractionKey], S>) => ExtractorFunction<DeepPartial<T[typeof extractionKey]>, S> =
        (input: TypeExtractor<T[typeof extractionKey], S>): ExtractorFunction<DeepPartial<T[typeof extractionKey]>, S> => {
            if (isPath(input)) {
                return jpv(input);
